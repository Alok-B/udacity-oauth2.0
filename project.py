...
# creates a flow object from the client secrets JSON file that is storing the client ID
# client secret, and other OAuth parameters
from oauth2client.client import flow_from_clientsecrets

# if we run into an error trying to exchange an authorization code for an access token
# we can use the flow exchange error method to catch it.
from oauth2client.client import FlowExchangeError
import httplib2
import json

# converts the returned value from a function into a real response that we
# can send off to our client
from flask import make_response
import requests

CLIENT_ID = json.loads(
  open('client_secrets.json', 'r').read())['web']['client_id']
APPLICATION_NAME = "Restaurant Menu App Credentials"
